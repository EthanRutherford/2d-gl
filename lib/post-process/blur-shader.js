...
class MotionBlurShader extends PostProcessShader {
	...
	deblur(renderable) {
		...
		this.prevMVs.delete(renderable);
	}
	render(renderable, frameNumber) {
		const gl = this.gl;
		const params = renderable.params;

		const prevEntry = this.prevMVs.get(renderable);
		const hasPrevMvm = prevEntry != null && prevEntry.frameNumber === frameNumber;
		const prevMvm = hasPrevMvm ? prevEntry.mvm : this.mvm;
		this.gl.uniformMatrix4fv(this.uPrevMVM, false, prevMvm);
		...
		this.prevMVs.set(renderable, {frameNumber: frameNumber + 1, mvm: this.mvm});
	}
}
