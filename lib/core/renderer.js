...
function computeRenderableMap(items) {
	const all = items.reduce((a, i) => computeChildren(a, i), []).sort(
		(a, b) => a.item.zIndex - b.item.zIndex || a.item.id - b.item.id,
	);

	const map = new Map();
	for (let i = 0; i < all.length; i++) {
		const r = all[i];
		if (!map.has(r.item.shader)) {
			map.set(r.item.shader, [r]);
		} else {
			map.get(r.item.shader).push(r);
		}
...
class Renderer {
	...
	render(camera, scene) {
		...
		const groups = computeRenderableMap(visible);
