...
class Renderer {
	...
	getInstance(shape) {
		const shader = this.shaders.get(shape.shaderType);
		return shape.getInstance(this.gl, shader);
	}
	resize() {
	...
	viewportToWorld(x, y, camera) {
		const xNormalized = (x / this.canvas.width) - .5;
		const yNormalized = .5 - (y / this.canvas.height);
